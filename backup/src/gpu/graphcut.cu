...
namespace ginf {
	...
	__global__ void gpuGraphCutConstructGraphKernel(GpuGrid<T> *grid, int *f, int *height, T *excess, T *capacity, int alpha, int beta) {
		...
		if (dim.isValid(x, y) && (f[dim.idx(x, y)] == alpha || f[dim.idx(x, y)] == beta)) {
			...
			excess[dim.idx(x, y)] = grid->getDataCost(x, y, alpha) + alphaCost;
	...
	template <typename T>
	bool isDone(Grid<T> *grid, Matrix<int> *excess, Matrix<int> *height) {
		for (int y = 0; y < grid->getHeight(); y++) {
			for (int x = 0; x < grid->getWidth(); x++) {	
				if (excess->at(x, y) && height->at(x, y) < grid->getNumNodes() + 2) return false;
			}
		}
		return true;
	}
			...
			for (int alpha = 0; alpha < grid->getNumLabels(); alpha++) {
				for (int beta = alpha + 1; beta < grid->getNumLabels(); beta++) {
					...
					while (true) {
						...
						//int numActive = buildActiveTileList(&isActive, activeList);
						//printf("active = %d\n", numActive);
						if (isDone(grid, &excess, &height)) break;
